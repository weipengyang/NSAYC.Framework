...
var bootstrap = function ($, learun) {
    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#wizard').wizard().on('change', function (e, data) {
                ...
                if (data.direction == "next") {
                    if (data.step == 1) {
                        ...
                        NSAYC.httpAsyncPost(top.$.rootUrl + '/CodeGeneratorModule/TemplatePC/LookWorkflowCode', postData, function (res) {
                            if (res.code == 200) {
            ...
            $('#dbId').lrselect({
                ...
                select: function (item) {
                    ...
                    if (item.hasChildren) {
                        ...
                        // 获取数据的表数据 
                        NSAYC.httpAsync('GET', top.$.rootUrl + '/SystemModule/DatabaseTable/GetList', { databaseLinkId: databaseLinkId }, function (data) {
                            dbAllTable = data;
                        });
                    }
                }
            ...
            $('#lr_db_add').on('click', function () {
                if (!!databaseLinkId) {
                    ...
                    NSAYC.layerForm({
                        id: 'DataTableForm',
                        title: '添加数据表',
                        url: top.$.rootUrl + '/CodeGeneratorModule/TemplatePC/DataTableForm?dbId=' + databaseLinkId,
                        width: 600,
                        height: 400,
                            ...
                            return top[id].acceptClick(function (data) {
                                for (var i = 0, l = dbTable.length; i < l; i++) {
                                    if (dbTable[i].name == data.name) {
                                        NSAYC.alert.warning('不能重复添加表');
                                        return false;
                                    }
                                    if (!data.relationName && !dbTable[i].relationName) {
                                        NSAYC.alert.warning('不能有两张主表');
                                        return false;
                                    }
                                }
            ...
            $('#lr_edit_tabs').on('click', function () {// 编辑选项卡
                NSAYC.layerForm({
                    id: 'TabEditIndex',
                    title: '编辑选项卡',
                    url: top.$.rootUrl + '/CodeGeneratorModule/TemplatePC/TabEditIndex',
                    width: 600,
                    height: 400,
                    maxmin: true,
                    btn: null,
            ...
            $('#lr_edit_form').on('click', function () {
                ...
                if (NSAYC.checkrow(_id)) {
                    NSAYC.layerForm({
                        id: 'FormSetting',
                        title: '编辑表单字段',
                        url: top.$.rootUrl + '/CodeGeneratorModule/TemplatePC/FormSetting?id=' + _id,
                        width: 400,
                        height: 520,
                        callBack: function (id) {
                            return top[id].acceptClick(function (data) {
            ...
            $('#lr_delete_form').on('click', function () {
                ...
                if (NSAYC.checkrow(_id)) {
                    NSAYC.layerConfirm('是否确认删除该字段', function (res, index) {
                        if (res) {
                            if (_type == 'gridtable') {
                                $.each(componts, function (id, item) {
                                    if (item.id == _id) {
            ...
            $('#lr_gridadd_form').on('click', function () {
                NSAYC.layerForm({
                    ...
                    title: '添加表格',
                    url: top.$.rootUrl + '/CodeGeneratorModule/TemplatePC/AddGridFieldIndex',
                    width: 400,
                    height: 300,
                    callBack: function (id) {
                        return top[id].acceptClick(function (data) {
            ...
            $('#lr_gridedit_form').on('click', function () {
                ...
                if (NSAYC.checkrow(_id)) {
                    NSAYC.layerForm({
                        id: 'GridFieldIndex',
                        title: '编辑表格',
                        url: top.$.rootUrl + '/CodeGeneratorModule/TemplatePC/GridFieldIndex?id=' + _id,
                        width: 700,
                        height: 500,
                        maxmin: true,
                        btn: null,
            ...
            $('#lr_preview_form').on('click', function () {
                ...
                NSAYC.layerForm({
                    id: 'PreviewForm',
                    title: '预览排版',
                    url: top.$.rootUrl + '/CodeGeneratorModule/TemplatePC/PreviewForm?formId=layer_CustmerCodeIndex',
                    width: w,
                    height: h,
                    maxmin: true,
                    btn: null
            ...
            $('#lr_add_query').on('click', function () {
                NSAYC.layerForm({
                    id: 'QueryFieldForm',
                    title: '添加条件字段',
                    url: top.$.rootUrl + '/CodeGeneratorModule/TemplatePC/QueryFieldForm',
                    height: 300,
                    width: 400,
                    callBack: function (id) {
                        return top[id].acceptClick(function (data) {
            ...
            $('#lr_edit_query').on('click', function () {
                ...
                if (NSAYC.checkrow(id)) {
                    NSAYC.layerForm({
                        id: 'QueryFieldForm',
                        title: '添加条件字段',
                        url: top.$.rootUrl + '/CodeGeneratorModule/TemplatePC/QueryFieldForm?id=' + id,
                        height: 300,
                        width: 400,
                        callBack: function (id) {
                            return top[id].acceptClick(function (data) {
            ...
            $('#lr_edit_col').on('click', function () {
                ...
                if (NSAYC.checkrow(compontId)) {
                    NSAYC.layerForm({
                        id: 'ColFieldForm',
                        title: '编辑条件字段',
                        url: top.$.rootUrl + '/CodeGeneratorModule/TemplatePC/ColFieldForm?compontId=' + compontId,
                        height: 300,
                        width: 400,
                        callBack: function (id) {
                            return top[id].acceptClick(function (data) {
        ...
        save: function () {
            ...
            $.lrSaveForm(top.$.rootUrl + '/CodeGeneratorModule/TemplatePC/CreateWorkflowCode', postData, function (res) { }, true);
        }
    };
