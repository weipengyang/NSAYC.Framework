...
var bootstrap = function ($, learun) {
    ...
    var page = {
        ...
        bind: function () {
            // 加载自定义流程列表
            NSAYC.httpAsync('GET', top.$.rootUrl + '/WorkFlowModule/WfScheme/GetCustmerSchemeInfoList', {}, function (data) {
                schemeList = data;
                render();
                isLoaded = true;
            });
        ...
        initData: function () {
            if (!!selectedRow) {
                ...
                NSAYC.httpAsync('GET', top.$.rootUrl + '/WorkFlowModule/WfDelegateRule/GetRelationList', { keyValue: keyValue }, function (data) {
    ...
    acceptClick = function (callBack) {
        ...
        if (schemeInfoList.length == 0) {
            NSAYC.alert.warning('至少选择一个流程模板');
            return false;
        }
        $.lrSaveForm(top.$.rootUrl + '/WorkFlowModule/WfDelegateRule/SaveForm?keyValue=' + keyValue, { 'strEntity': JSON.stringify(formData), 'strSchemeInfo': String(schemeInfoList) }, function (res) {
            if (!!callBack) {
                callBack();
            }
        });
