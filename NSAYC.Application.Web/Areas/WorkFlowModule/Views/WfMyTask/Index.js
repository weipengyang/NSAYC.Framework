    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#lr_eye').on('click', function () {
                ...
                if (NSAYC.checkrow(processId)) {
                    NSAYC.frameTab.open({ F_ModuleId: processId + taskId, F_Icon: 'fa magic', F_FullName: '查看流程进度【' + processName + '】', F_UrlAddress: '/WorkFlowModule/WfMyTask/CustmerWorkFlowForm?tabIframeId=' + processId + taskId + '&type=100' + "&processId=" + processId + "&taskId=" + taskId });
                }
            });
            // 发起流程
            $('#lr_release').on('click', function () {
                NSAYC.layerForm({
                    id: 'form',
                    title: '选择流程模板',
                    url: top.$.rootUrl + '/WorkFlowModule/WfMyTask/ReleaseForm',
                    height: 600,
                    width: 825,
                    maxmin: true,
            ...
            $('#lr_verify').on('click', function () {
                ...
                if (taskType == 4) {
                    if (NSAYC.checkrow(taskId)) {
                        NSAYC.frameTab.open({ F_ModuleId: taskId, F_Icon: 'fa magic', F_FullName: '审核流程【' + processName + '/' + taskName + '】', F_UrlAddress: '/WorkFlowModule/WfMyTask/CustmerWorkFlowForm?tabIframeId=' + taskId + '&type=4' + "&processId=" + processId + "&taskId=" + taskId });
                    }
                }
                else if (taskType == 1) {
                    if (NSAYC.checkrow(taskId)) {
                        NSAYC.frameTab.open({ F_ModuleId: taskId, F_Icon: 'fa magic', F_FullName: '审核流程【' + processName + '/' + taskName + '】', F_UrlAddress: '/WorkFlowModule/WfMyTask/CustmerWorkFlowForm?tabIframeId=' + taskId + '&type=1' + "&processId=" + processId + "&taskId=" + taskId });
                    }
                }
                else if (taskType == 2) {
                    NSAYC.alert.warning('请点击重新发起');
                }
                else {
                    if (NSAYC.checkrow(taskId)) {
                        NSAYC.frameTab.open({ F_ModuleId: taskId, F_Icon: 'fa magic', F_FullName: '审核流程【' + processName + '/' + taskName + '】', F_UrlAddress: '/WorkFlowModule/WfMyTask/CustmerWorkFlowForm?tabIframeId=' + taskId + '&type=3' + "&processId=" + processId + "&taskId=" + taskId });
                    }
                }
            });
        },
                        ...
                        formatter: function (cellvalue, row, dfop, $cell) {
                            ...
                            if (isaAain) {
                                $cell.on('click', function () {
                                    NSAYC.frameTab.open({ F_ModuleId: row.F_Id, F_Icon: 'fa magic', F_FullName: '重新发起流程【' + row.F_ProcessName + '】', F_UrlAddress: '/WorkFlowModule/WfMyTask/CustmerWorkFlowForm?processId=' + row.F_Id + '&tabIframeId=' + row.F_Id + '&type=2' });
                                });
