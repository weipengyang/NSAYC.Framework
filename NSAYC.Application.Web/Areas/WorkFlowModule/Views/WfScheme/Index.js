    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#lr_add').on('click', function () {
                NSAYC.layerForm({
                    id: 'Form',
                    title: '新增流程模板',
                    url: top.$.rootUrl + '/WorkFlowModule/WfScheme/Form?categoryId=' + categoryId,
                    width: 1200,
                    height: 900,
                    maxmin: true,
                    btn: null
            ...
            $('#lr_edit').on('click', function () {
                ...
                if (NSAYC.checkrow(keyValue)) {
                    NSAYC.layerForm({
                        id: 'Form',
                        title: '编辑流程模板-' + name,
                        url: top.$.rootUrl + '/WorkFlowModule/WfScheme/Form?keyValue=' + keyValue + '&categoryId=' + categoryId + '&shcemeCode=' + code,
                        width: 1200,
                        height: 900,
                        maxmin: true,
                        btn: null
            ...
            $('#lr_delete').on('click', function () {
                ...
                if (NSAYC.checkrow(keyValue)) {
                    NSAYC.layerConfirm('是否确认删除该项！', function (res) {
                        if (res) {
                            NSAYC.deleteForm(top.$.rootUrl + '/WorkFlowModule/WfScheme/DeleteForm', { keyValue: keyValue }, function () {
                                refreshGirdData();
                            });
                        }
                    });
                ...
                if (NSAYC.checkrow(keyValue)) {
                    if (enabledMark != 1) {
                        NSAYC.layerConfirm('是否确认启用该表单模板！', function (res) {
                            if (res) {
                                NSAYC.postForm(top.$.rootUrl + '/WorkFlowModule/WfScheme/UpDateSate', { keyValue: keyValue, state: 1 }, function () {
                                    refreshGirdData();
                                });
                            }
                        });
                ...
                if (NSAYC.checkrow(keyValue)) {
                    if (enabledMark == 1) {
                        NSAYC.layerConfirm('是否确认禁用该表单模板！', function (res) {
                            if (res) {
                                NSAYC.postForm(top.$.rootUrl + '/WorkFlowModule/WfScheme/UpDateSate', { keyValue: keyValue, state: 0 }, function () {
                                    refreshGirdData();
                                });
                            }
                        });
            ...
            $('#lr_history').on('click', function () {
                ...
                if (NSAYC.checkrow(keyValue)) {
                    NSAYC.layerForm({
                        id: 'HistoryForm',
                        title: '流程模板历史记录-' + Name,
                        url: top.$.rootUrl + '/WorkFlowModule/WfScheme/HistoryForm?&keyValue=' + keyValue,
                        width: 600,
                        height: 400,
                        maxmin: true,
                        btn: null
            ...
            $('#lr_category').on('click', function () {
                NSAYC.layerForm({
                    id: 'ClassifyIndex',
                    title: '分类管理',
                    url: top.$.rootUrl + '/SystemModule/DataItem/DetailIndex?itemCode=FlowSort',
                    width: 800,
                    height: 500,
                    maxmin: true,
                    btn: null,
        ...
        initGird: function () {
            $('#girdtable').lrAuthorizeJfGrid({
                url: top.$.rootUrl + '/WorkFlowModule/WfScheme/GetSchemeInfoPageList',
                headData: [
