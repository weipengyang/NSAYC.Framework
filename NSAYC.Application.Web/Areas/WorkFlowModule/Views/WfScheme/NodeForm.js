        ...
        if (!!formId) {
            if (type == 0) {
                if (!!formcomponts[formId]) {
                    ...
                    formcomponts[formId] = [];
                    $.lrSetForm(top.$.rootUrl + '/FormModule/Custmerform/GetFormData?keyValue=' + formId, function (data) {
                            ...
                            for (var j = 0, jl = componts.length; j < jl; j++) {
                                ...
                                if (compont.type == 'girdtable') {
                                    $.each(compont.fieldsData, function (_i, _item) {
                                        if (_item.type != 'guid')
                                        {
                                            var point = { id: NSAYC.newGuid(), formId: formId, formName: formName, fieldName: compont.title + '-' + _item.name, fieldId: compont.id + '|' + _item.id, isLook: '1', isEdit: '1' };
                                ...
                                {
                                    var point = { id: NSAYC.newGuid(), formId: formId, formName: formName, fieldName: compont.title, fieldId: compont.id, isLook: '1', isEdit: '1' };
                                    formcomponts[formId].push(point);
                                    authorize.push(point);
                                }
                            }
    ...
    function isRepeat(id) {
        ...
        for (var i = 0, l = auditors.length; i < l; i++) {
            if (auditors[i].auditorId == id) {
                NSAYC.alert.warning('重复添加审核人员信息');
                res = true;
                break;
            }
        }
        ...
        bind: function () {
            ...
            $('#lr_delete_auditor').on('click', function () {
                ...
                if (NSAYC.checkrow(_id)) {
                    NSAYC.layerConfirm('是否确认删除该审核人员！', function (res,index) {
                        if (res) {
                            for (var i = 0, l = auditors.length; i < l; i++) {
            ...
            $('#lr_add_workform').on('click', function () {
                NSAYC.layerForm({
                    id: 'WorkformForm',
                    title: '添加表单',
                    url: top.$.rootUrl + '/WorkFlowModule/WfScheme/WorkformForm',
                    width: 400,
                    height: 320,
                    callBack: function (id) {
                        return top[id].acceptClick(function (data) {
            ...
            $('#lr_edit_workform').on('click', function () {
                ...
                if (NSAYC.checkrow(_id)) {
                    NSAYC.layerForm({
                        id: 'WorkformForm',
                        title: '编辑表单',
                        url: top.$.rootUrl + '/WorkFlowModule/WfScheme/WorkformForm?id=' + _id,
                        width: 400,
                        height: 300,
                        callBack: function (id) {
                            return top[id].acceptClick(function (data) {
            ...
            $('#lr_delete_workform').on('click', function () {
                var _id = $('#workform_girdtable').jfGridValue('id');
                if (NSAYC.checkrow(_id)) {
                    NSAYC.layerConfirm('是否确认删除该表单！', function (res, index) {
                        if (res) {
                            for (var i = 0, l = workforms.length; i < l; i++) {
            ...
            $('#lr_add_authorize').on('click', function () {
                NSAYC.layerForm({
                    id: 'AuthorizeForm',
                    title: '添加表单权限字段',
                    url: top.$.rootUrl + '/WorkFlowModule/WfScheme/AuthorizeForm',
                    width: 400,
                    height: 340,
                    callBack: function (id) {
                        return top[id].acceptClick(function (data) {
            ...
            $('#lr_edit_authorize').on('click', function () {
                ...
                if (NSAYC.checkrow(_id)) {
                    NSAYC.layerForm({
                        id: 'AuthorizeForm',
                        title: '编辑表单权限字段',
                        url: top.$.rootUrl + '/WorkFlowModule/WfScheme/AuthorizeForm?id=' + _id,
                        width: 400,
                        height: 340,
                        callBack: function (id) {
                            return top[id].acceptClick(function (data) {
            ...
            $('#lr_delete_authorize').on('click', function () {
                var _id = $('#authorize_girdtable').jfGridValue('id');
                if (NSAYC.checkrow(_id)) {
                    NSAYC.layerConfirm('是否确认删除该权限字段！', function (res, index) {
                        if (res) {
                            for (var i = 0, l = authorize.length; i < l; i++) {
                                if (authorize[i].id == _id) {
            ...
            $('#lr_add_condition').on('click', function () {
                NSAYC.layerForm({
                    id: 'AuthorizeForm',
                    title: '添加条件字段',
                    url: top.$.rootUrl + '/WorkFlowModule/WfScheme/ConditionFieldForm',
                    width: 400,
                    height: 300,
                    callBack: function (id) {
                        return top[id].acceptClick(function (data) {
            ...
            $('#lr_edit_condition').on('click', function () {
                ...
                if (NSAYC.checkrow(_id)) {
                    NSAYC.layerForm({
                        id: 'AuthorizeForm',
                        title: '编辑条件字段',
                        url: top.$.rootUrl + '/WorkFlowModule/WfScheme/ConditionFieldForm?id=' + _id,
                        width: 400,
                        height: 300,
                        callBack: function (id) {
                            return top[id].acceptClick(function (data) {
            ...
            $('#lr_delete_condition').on('click', function () {
                var _id = $('#condition_girdtable').jfGridValue('id');
                if (NSAYC.checkrow(_id)) {
                    NSAYC.layerConfirm('是否确认删除该条件字段！', function (res, index) {
                        if (res) {
                            for (var i = 0, l = conditions.length; i < l; i++) {
                                if (conditions[i].id == _id) {
