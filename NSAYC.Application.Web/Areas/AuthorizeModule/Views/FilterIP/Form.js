...
var bootstrap = function ($, learun) {
    ...
    var page = {
        ...
        initData: function () {
            if (!!keyValue) {
                $.lrSetForm(top.$.rootUrl + '/AuthorizeModule/FilterIP/GetFormData?keyValue=' + keyValue, function (data) {
    ...
    acceptClick = function (callBack) {
        ...
        if (compareIP(data.F_StartIP, data.F_EndIP) == -1) {
            NSAYC.alert.error('IP不在同一个网段内');
            return false;
        }
        if (compareIP(data.F_StartIP, data.F_EndIP) == 0) {
            NSAYC.alert.error('结束IP不能大于开始IP');
            return false;
        }
        var postData = {
        ...
        $.lrSaveForm(top.$.rootUrl + '/AuthorizeModule/FilterIP/SaveForm?keyValue=' + keyValue, postData, function (res) {
            // 保存成功后才回调
            if (!!callBack) {
                callBack();
            }
