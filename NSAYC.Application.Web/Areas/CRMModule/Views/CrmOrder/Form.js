...
var bootstrap = function ($, learun) {
    ...
    var acceptClick = function (type) {// 0保存并新增 1保存
        ...
        NSAYC.layerConfirm('注：您确认要保存此操作吗？', function (res, index) {
            if (res) {
                $.lrSaveForm(top.$.rootUrl + '/CRMModule/CrmOrder/SaveForm?keyValue=' + keyValue, postData, function (res) {
                    if (res.code == 200) {
                        if (type == 0) {
                            window.location.href = top.$.rootUrl + '/CRMModule/CrmOrder/Form';
                        }
                        else {
                            NSAYC.frameTab.close('order_add');
                        }
                    }
                });
    ...
    var page = {
        ...
        initData: function () {
            if (!!keyValue) {
                $.lrSetForm(top.$.rootUrl + '/CRMModule/CrmOrder/GetFormData?keyValue=' + keyValue, function (data) {//
