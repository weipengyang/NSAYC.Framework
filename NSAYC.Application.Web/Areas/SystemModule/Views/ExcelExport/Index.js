            ...
            $('#lr_edit').on('click', function () {
                ...
                if (NSAYC.checkrow(keyValue)) {
                    NSAYC.layerForm({
                        id: 'Form',
                        title: '编辑快速导出',
                        url: top.$.rootUrl + '/SystemModule/ExcelExport/Form',
                        width: 500,
                        height: 300,
                        callBack: function (id) {
                            return top[id].acceptClick(refreshGirdData);
            ...
            $('#lr_delete').on('click', function () {
                ...
                if (NSAYC.checkrow(keyValue)) {
                    NSAYC.layerConfirm('是否确认删除该项！', function (res) {
                        if (res) {
                            NSAYC.deleteForm(top.$.rootUrl + '/SystemModule/ExcelExport/DeleteForm', { keyValue: keyValue }, function () {
                                refreshGirdData();
                            });
                        }
                    });
            ...
            $('#lr_enable').on('click', function () {
                ...
                if (NSAYC.checkrow(keyValue)) {
                    NSAYC.layerConfirm('是否确认要【启用】！', function (res) {
                        if (res) {
                            NSAYC.postForm(top.$.rootUrl + '/SystemModule/ExcelExport/UpdateState', { keyValue: keyValue, state: 1 }, function () {
                                refreshGirdData();
                            });
                        }
                    });
            ...
            $('#lr_disable').on('click', function () {
                ...
                if (NSAYC.checkrow(keyValue)) {
                    NSAYC.layerConfirm('是否确认要【停用】！', function (res) {
                        if (res) {
                            NSAYC.postForm(top.$.rootUrl + '/SystemModule/ExcelExport/UpdateState', { keyValue: keyValue, state: 0 }, function () {
                                refreshGirdData();
                            });
                        }
                    });
