...
var bootstrap = function ($, learun) {
        ...
        bind: function () {
            $('#lr_add_format').on('click', function () {
                ...
                NSAYC.layerForm({
                    id: 'FormatForm',
                    title: '添加',
                    url: top.$.rootUrl + '/SystemModule/CodeRule/FormatForm',
                    width: 450,
                    height: 310,
            ...
            $('#F_EnCode').on('blur', function () {
                $.lrExistField(keyValue, 'F_EnCode', top.$.rootUrl + '/SystemModule/CodeRule/ExistEnCode');
            });
            $('#F_FullName').on('blur', function () {
                $.lrExistField(keyValue, 'F_FullName', top.$.rootUrl + '/SystemModule/CodeRule/ExistFullName');
            });
        },
        initData: function () {
            if (!!selectedRow) {
    ...
    var mergeFileChunks = function (file) {
        ...
        param['ext'] = file.ext;
        param['templateId'] = id;
        NSAYC.httpAsyncPost(top.$.rootUrl + "/SystemModule/ExcelImport/ExecuteImportExcel", param, function (res) {
            var $fileItem = $('#lr_form_file_queue_list').find('#lr_filequeue_' + file.id);
            $fileItem.find('.lr-uploader-progress').remove();
            if (res.code == NSAYC.httpCode.success) {
                if (res.data.Success != '0') {
                    NSAYC.alert.success('导入成功' + res.data.Success + '条');
                }
                // 文件保存成功后
                ...
                if (res.data.Fail != '0')
                {
                    NSAYC.download({ url: top.$.rootUrl + '/SystemModule/ExcelImport/DownImportErrorFile', param: { fileId: fileInfo[file.id].fileGuid, fileName: fileInfo[file.id].name, __RequestVerificationToken: $.lrToken }, method: 'POST' });
                }
            }
            else {
    ...
    var reomveFileChunks = function (file) {
        ...
        param['chunks'] = fileInfo[file.id].chunks;
        NSAYC.httpAsyncPost(top.$.rootUrl + "/SystemModule/Annexes/MergeAnnexesFile", param, function (res) { });
        var $fileItem = $('#lr_form_file_queue_list').find('#lr_filequeue_' + file.id);
        $fileItem.find('.lr-uploader-progress').remove();
        $fileItem.append('<div class="lr-msg"><i class="fa fa-exclamation-circle"></i></div>');
    }
    ...
    var page = {
        ...
        init: function () {
            ...
            $('#lr_down_file_btn').on('click', function () {
                NSAYC.download({ url: top.$.rootUrl + '/SystemModule/ExcelImport/DownSchemeFile', param: { keyValue: id, __RequestVerificationToken: $.lrToken }, method: 'POST' });
            });
            ...
            page.uploader = WebUploader.create({
                ...
                // 文件接收服务端。
                server: top.$.rootUrl + "/SystemModule/Annexes/UploadAnnexesFileChunk",
                // 选择文件的按钮。可选。
        ...
        fileQueued: function (file) {// 文件加载到队列
            ...
            $item.append('<span class="lr-file-name">' + file.name + '(' + NSAYC.countFileSize(file.size) + ')</span>');
