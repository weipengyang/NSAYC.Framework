...
var bootstrap = function ($, learun) {
    ...
    var page = {
            ...
            $('#lr_dbtree').lrtree({
                ...
                nodeClick: function (item) {
                    if (!item.hasChildren) {
                        dbTable = item.text;
                        dbId = item.value;
                        NSAYC.httpAsync('GET', top.$.rootUrl + '/SystemModule/DatabaseTable/GetFieldTreeList', { databaseLinkId: item.value, tableName: item.text }, function (res) {
                            cols.length = 0;
                            $('#girdtable').jfGridSet('refreshdata', { rowdatas: cols });
        ...
        initData: function () {
            ...
            if (!!keyValue) {
                $.lrSetForm(top.$.rootUrl + '/SystemModule/ExcelImport/GetFormData?keyValue=' + keyValue, function (data) {//
                    ...
                    NSAYC.httpAsync('GET', top.$.rootUrl + '/SystemModule/DatabaseTable/GetFieldTreeList', { databaseLinkId: dbId, tableName: dbTable }, function (res) {
                        var map = {};
    ...
    acceptClick = function (callBack) {
        ...
        $.lrSaveForm(top.$.rootUrl + '/SystemModule/ExcelImport/SaveForm', postData, function (res) {
            // 保存成功后才回调
            if (!!callBack) {
                callBack();
            }
