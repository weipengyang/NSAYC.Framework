    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#lr_add').on('click', function () {
                if (!moduleId) {
                    NSAYC.alert.warning('请选择功能！');
                    return false;
                }
                NSAYC.layerForm({
                    id: 'Form',
                    title: '添加快速导入',
                    url: top.$.rootUrl + '/SystemModule/ExcelImport/Form?moduleId=' + moduleId,
                    width: 1100,
                    height: 700,
                    maxmin: true,
            ...
            $('#lr_edit').on('click', function () {
                ...
                if (NSAYC.checkrow(keyValue)) {
                    NSAYC.layerForm({
                        id: 'Form',
                        title: '编辑快速导入',
                        url: top.$.rootUrl + '/SystemModule/ExcelImport/Form?keyValue=' + keyValue,
                        width: 1100,
                        height: 700,
                        maxmin: true,
                        callBack: function (id) {
            ...
            $('#lr_delete').on('click', function () {
                ...
                if (NSAYC.checkrow(keyValue)) {
                    NSAYC.layerConfirm('是否确认删除该项！', function (res) {
                        if (res) {
                            NSAYC.deleteForm(top.$.rootUrl + '/SystemModule/ExcelImport/DeleteForm', { keyValue: keyValue }, function () {
                                refreshGirdData();
                            });
                        }
                    });
            ...
            $('#lr_enable').on('click', function () {
                var keyValue = $('#girdtable').jfGridValue('F_Id');
                if (NSAYC.checkrow(keyValue)) {
                    NSAYC.layerConfirm('是否确认要【启用】！', function (res) {
                        if (res) {
                            NSAYC.postForm(top.$.rootUrl + '/SystemModule/ExcelImport/UpdateForm?keyValue=' + keyValue, { F_EnabledMark: 1 }, function () {
                                refreshGirdData();
                            });
                        }
                    });
            ...
            $('#lr_disable').on('click', function () {
                var keyValue = $('#girdtable').jfGridValue('F_Id');
                if (NSAYC.checkrow(keyValue)) {
                    NSAYC.layerConfirm('是否确认要【停用】！', function (res) {
                        if (res) {
                            NSAYC.postForm(top.$.rootUrl + '/SystemModule/ExcelImport/UpdateForm?keyValue=' + keyValue, { F_EnabledMark: 0 }, function () {
                                refreshGirdData();
                            });
                        }
                    });
                ...
                headData: [
                    { label: "模板名称", name: "F_Name", width: 160, align: "left" },
                    {
                        ...
                        formatter: function (cellvalue) {
                            var data = NSAYC.clientdata.get(['modulesMap']);
