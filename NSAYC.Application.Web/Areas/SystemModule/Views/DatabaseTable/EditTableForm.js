        ...
        bind: function () {
                ...
                NSAYC.layerForm({
                    ...
                    callBack: function (id) {
                        return top[id].acceptClick(function (data) {
                            ...
                            NSAYC.httpAsyncGet(top.$.rootUrl + '/SystemModule/DatabaseTable/GetFieldList?databaseLinkId=' + data.databaseLinkId + '&tableName=' + data.tableName, function (res) {
            ...
            $('#lr_copyrow').on('click', function () {
                ...
                if (selectedRow && selectedRow.length > 0) {
                    ...
                    top.NSAYC.alert.warning('请选择需要复制的行!');
                }
            });
            ...
            $('#lr_savedraft').on('click', function () {
                ...
                $.lrSaveForm(top.$.rootUrl + '/SystemModule/DatabaseTable/SaveDraft?keyValue=' + draftId, postData, function (res) {
                    // 保存成功后才回调
                    console.log(res);
                    if (res.code == 200) {
                        draftId = res.data;
            ...
            $('#lr_release').on('click', function () {
                ...
                $.lrSaveForm(top.$.rootUrl + '/SystemModule/DatabaseTable/SaveTable', postData, function (res) {
                    // 新建成功后刷新页面
                    if (res.code == 200) {
                        draftId = '';
        ...
        checkGrid: function (data) {
            ...
            $.each(data, function (_index, _item) {
                if (_item.f_column === undefined || _item.f_column === '') {
                    NSAYC.alert.error('【第' + _index + '列】列名为空');
                    flag = false;
                    return false;
                }
                else if (_item.f_datatype === undefined || _item.f_datatype === '') {
                    NSAYC.alert.error('【第' + _index + '列】数据类型为空');
                    flag = false;
                    return false;
                }
                else if (_item.f_datatype == 'varchar' && (_item.f_length == 0 || _item.f_length == undefined)) {
                    NSAYC.alert.error('【第' + _index + '列】字串长度设置错误');
                    flag = false;
                    return false;
                }
                else if (map[_item.f_column] != undefined ){
                    NSAYC.alert.error('【第' + _index + '列与第' + map[_item.f_column] + '列】列名重复');
                    flag = false;
                    return false;
                }
