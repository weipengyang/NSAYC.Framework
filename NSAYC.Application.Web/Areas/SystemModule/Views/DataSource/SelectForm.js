    ...
    var page = {
            ...
            $('#girdtable').jfGrid({
                ...
                onSelectRow: function (row) {
                    if (valuelist[0] != row.F_Code)
                    {
                        ...
                        page.setValue();
                        //获取字段数据
                        NSAYC.httpAsync('GET', top.$.rootUrl + '/SystemModule/DataSource/GetDataColName', { code: row.F_Code }, function (data) {
                            var fieldData = [];
        ...
        initData: function () {
            if (!!selectValue) {
                ...
                NSAYC.httpAsync('GET', top.$.rootUrl + '/SystemModule/DataSource/GetDataColName', { code: valuelist[0] }, function (data) {
                    var fieldData = [];
                    for (var i = 0, l = data.length; i < l; i++) {
                        var id = data[i];
                        var selectpoint = { value: id, text: id };
