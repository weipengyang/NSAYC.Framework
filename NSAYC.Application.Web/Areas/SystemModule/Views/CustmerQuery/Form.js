...
var bootstrap = function ($, learun) {
    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#F_ModuleId').lrselect({
                ...
                var module = NSAYC.clientdata.get(['modulesMap', moduleId]);
            ...
            $('#querylist').on('click', function (e) {
                ...
                if ($et.hasClass('btn-edit')) {
                    ...
                    NSAYC.layerForm({
                        id: 'QueryForm',
                        title: '添加自定义查询条件',
                        url: top.$.rootUrl + '/SystemModule/CustmerQuery/QueryForm?moduleId=' + moduleId + '&rowid=' + _rowid,
                        width: 500,
                        height: 300,
            ...
            $('#lr_query_add').on('click', function () {
                if (!moduleId) {
                    NSAYC.alert.warning('请选择所属功能！');
                    return false;
                }
                NSAYC.layerForm({
                    id: 'QueryForm',
                    title: '添加自定义查询条件',
                    url: top.$.rootUrl + '/SystemModule/CustmerQuery/QueryForm?moduleId=' + moduleId,
                    width: 500,
                    height: 300,
                    callBack: function (id) {
                        return top[id].acceptClick(refreshData);
    ...
    acceptClick = function (callBack) {
        ...
        $.lrSaveForm(top.$.rootUrl + '/SystemModule/CustmerQuery/SaveForm?keyValue=' + keyValue, postData, function (res) {
            // 保存成功后才回调
            if (!!callBack) {
                callBack();
            }
