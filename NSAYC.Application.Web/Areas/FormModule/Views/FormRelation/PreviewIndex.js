...
var bootstrap = function ($, learun) {
    ...
    var page = {
        init: function () {
            // 获取自定义表单设置内容
            NSAYC.httpAsync('GET', top.$.rootUrl + '/FormModule/FormRelation/GetCustmerFormData', { keyValue: id }, function (data) {
            ...
            $('#lr_add').on('click', function () {
                if (settingJson.layer.opentype == '1') {// 窗口弹层页
                    NSAYC.layerForm({
                        id: 'Form',
                        title: '新增',
                        url: top.$.rootUrl + '/FormModule/Custmerform/LayerInstanceForm?id=' + relation.F_FormId,
                    ...
                    NSAYC.frameTab.open({ F_ModuleId: id, F_Icon: 'fa fa-pencil-square-o', F_FullName: '新增', F_UrlAddress: '/FormModule/Custmerform/TabInstanceForm?id=' + relation.F_FormId });
                }
            });
            // 编辑
                ...
                if (NSAYC.checkrow(keyValue)) {
                    if (settingJson.layer.opentype == '1') {// 窗口弹层页
                        NSAYC.layerForm({
                            id: 'Form',
                            title: '编辑',
                            url: top.$.rootUrl + '/FormModule/Custmerform/LayerInstanceForm?id=' + relation.F_FormId + "&keyValue=" + keyValue,
                            width: settingJson.layer.width,
                            height: settingJson.layer.height,
                            callBack: function (id) {
                                return top[id].acceptClick(refreshGirdData);
                        ...
                        NSAYC.frameTab.open({ F_ModuleId: id, F_Icon: 'fa fa-pencil-square-o', F_FullName: '编辑', F_UrlAddress: '/FormModule/Custmerform/TabInstanceForm?id=' + relation.F_FormId + "&keyValue=" + keyValue });
                    }
                }
            });
            // 删除
            $('#lr_delete').on('click', function () {
                ...
                if (NSAYC.checkrow(keyValue)) {
                    NSAYC.layerConfirm('是否确认删除该项！', function (res) {
                        if (res) {
                            NSAYC.deleteForm(top.$.rootUrl + '/FormModule/Custmerform/DeleteInstanceForm', { keyValue: keyValue, schemeInfoId: relation.F_FormId }, function () {
                                refreshGirdData();
                            });
                        }
                    });
                    ...
                    switch (item.compont.type) {
                        ...
                        case 'select':
                            if (item.compont.dataSource == "0") {
                                ...
                                point.formatterAsync = function (callback, value, row) {
                                    NSAYC.clientdata.getAsync('sourceData', {
                                        key: value,
                                        keyId: vlist[2],
                                        code: vlist[0],
                                        callback: function (_data) {
