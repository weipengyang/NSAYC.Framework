...
namespace NSAYC.Dev.Tool
{
    public partial class 爱养车开发小工具 : Form
    {
        ...
        {
            string formPath = Config.GetValue("FormPath") + "\\NSAYC.Framework.Module\\";//来源文件目录
        ...
        {
            string formPath = Config.GetValue("FormPath") + "\\NSAYC.Application.Web\\";//来源文件目录
            ...
            foreach (string filePath in filePaths)
            {
                if (filePath.IndexOf("\\bin\\") == -1 && filePath.IndexOf("\\obj\\") == -1 && filePath.IndexOf("NSAYC.Application.Web.csproj") == -1)
                {
                    textBox1.AppendText(num + ":" + filePath + "\r\n");
                    string path = toPath + filePath.Replace(formPath, "");
